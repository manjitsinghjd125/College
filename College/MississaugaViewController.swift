//
//  MississaugaViewController.swift
//  College
//
...
class MississaugaViewController: UIViewController {
    ...
    @IBAction func submit(_ sender: Any) {
        ...
        if(UserName.text?.isEmpty == false || UserName.text?.isEmpty == false)
        {
            ...
            alertController.addAction(UIAlertAction(title: "Yes", style: UIAlertActionStyle.destructive) {
                ...
                MississaugaTableViewController.courses[self.index.row] = self.programname.text!
                MississaugaTableViewController.seatsavaliable[self.index.row] = ( Int(self.remainingseats.text!)! - 1)
